    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('hrd.pengajuan-cuti.update', $cuti->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="form-group mb-3">
                            ...
                            <select name="karyawan_id" id="karyawan_id" class="form-control @error('karyawan_id') is-invalid @enderror">
                                ...
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}" {{ old('karyawan_id', $cuti->karyawan_id) == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                                @endforeach
                            </select>
                            @error('karyawan_id')
